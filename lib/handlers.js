...
handlers._checks.post = function (data, callback) {
  // validate inputs
  let protocal =
    typeof data.payload.protocal == "string" &&
    ["http", "https"].indexOf(data.payload.protocal) > -1
      ? data.payload.protocal
      : false;
  ...
  let statusCodes =
    typeof data.payload.statusCodes == "object" &&
    data.payload.statusCodes instanceof Array &&
    data.payload.statusCodes.length > 0
      ? data.payload.statusCodes
      : false;

  let timeoutSeconds =
    typeof data.payload.timeoutSeconds == "number" &&
    data.payload.timeoutSeconds % 1 == 0 &&
    data.payload.timeoutSeconds.length > 1 &&
    data.payload.timeoutSeconds.length <= 5
      ? data.payload.timeoutSeconds.length
      : false;
  if (protocal && url && method && statusCodes && timeoutSeconds) {
    let token =
      typeof data.token.headers == "string" ? data.token.headers : false;
    _data.read("tokens", token, function (err, tokenData) {
      if (!err && tokenData) {
        let userPhone = tokenData.userPhone;
